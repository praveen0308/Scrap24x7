...
import com.jmm.brsap.scrap24x7.databinding.FragmentManagePickupRequestBinding
...
@AndroidEntryPoint
class ManagePickupRequest : BaseFragment<FragmentManagePickupRequestBinding>(FragmentManagePickupRequestBinding::inflate),
    ManagePickupRequestAdapter.ManagePickupRequestInterface,
    ...
    private lateinit var managePickupRequestAdapter: ManagePickupRequestAdapter
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.chipGroup.setOnCheckedChangeListener { group, checkedId ->
            ...
            when(checkedId){
                R.id.cp_all->{
                    viewModel.setSelectedPRType(FilterEnum.ALL)
                    managePickupRequestAdapter.setPickupRequestList(pickupRequests)
                }
                R.id.cp_raised->{
                    viewModel.setSelectedPRType(FilterEnum.RAISED)

                }
                R.id.cp_pickup->{
                    viewModel.setSelectedPRType(FilterEnum.PICKUP)
                }
    ...
    private fun setRvPickupRequests() {
        managePickupRequestAdapter = ManagePickupRequestAdapter(this)
...
//        request.pickup_id?.let { viewModel.updatePickupStatus(it,2) }
    ...
    override fun onItemClick(request: PickupRequest) {
        val intent = Intent(requireActivity(), PickupDetailActivity::class.java)
        intent.putExtra("USER_TYPE", UserEnum.ADMIN)
